...
public class PolishAirportsExample {
    ...
    private void runExample(boolean withWeightConstraints) {
        ...
        // Zapis wag SMAA do plików CSV (lotniska)
        var weights = smaaEfficiency.generateWeightSamples(data);
        String suffix = withWeightConstraints ? "_custom" : "";
        try {
            weights.getInputSamples().write().csv("airports_input_weights" + suffix + ".csv");
            weights.getOutputSamples().write().csv("airports_output_weights" + suffix + ".csv");
            System.out.println("Zapisano wagi SMAA do plików CSV (airports_input_weights" + suffix + ".csv oraz airports_output_weights" + suffix + ".csv)");
        } catch (Exception e) {
            System.err.println("Błąd zapisu wag do CSV: " + e.getMessage());
        }
